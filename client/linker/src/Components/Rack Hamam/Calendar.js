...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    const fetchData=async()=>{
        setIsLoading(true)
        try{
            const response = await axios.post("http://gmhotel.ir/api/getFixedReserves",{date:currentDate.locale('fa').format('YYYY-MM')})
  ...
  const modifyFixedReserves = async(e) =>{
    ...
    if (hamamStartHour && hamamEndHour) {
      ...
      try{
        setIsLoading(true)
        const response = await axios.post("http://gmhotel.ir/api/modifyFixedReserves",{
  ...
  const removeSpecificReserve = async(e)=>{
    ...
    try{
      ...
      const removeReserve = async() =>{
        setIsLoading(true)
        const removeResponse = await axios.post("http://gmhotel.ir/api/removeHamamReserve",{
          UniqueId:reserveDetails.UniqueId,
          FullName:reserveDetails.FullName,
          Phone:reserveDetails.Phone,
          Date:reserveDetails.Date,
